...
var Day = React.createClass({
  render: function() {
    ...
    return ( 
      <div className="day">
        <p className="day_num">{this.props.index + 1}</p>
      </div>
    );
  }
});

var ReservationDay = React.createClass({
  render: function() {
    ...
    return ( 
      <div className="reserve_day">
        <div className="reserve_count_wrapper">
          <p className="reservation_count">{this.props.reservations}</p>
        </div>
        <p className="day_num">{this.props.index + 1}</p>
      </div>
    );
...
var Month = React.createClass({
  render: function() {
    ...
    var days = this.props.data.map(function(day, i) {
      ...
      if (day.length > 0) {
        return <ReservationDay data={day} key={i} index={i} reservations={day.length} ></ReservationDay>
      }
